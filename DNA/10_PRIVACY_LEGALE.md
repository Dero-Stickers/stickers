...
- **Titolare del trattamento: Davide De Rose** — contatto dati/supporto **UNICO** su `stickers@deroarts.com` (email definitiva, alias Zoho attivo). Fonte unica: `app_settings.support_email` (pannello admin → Impostazioni). Hook condiviso `useSupportEmail()` (fallback `stickers@deroarts.com`) usato da Account bloccato, firma Profilo, info Pagamenti (MatchDetail). Nei testi legali il segnaposto **`{EMAIL_SUPPORTO}`** è sostituito al render in `LegalPage`: cambiare l'email in admin la aggiorna ovunque, testi legali inclusi.
